...
/*
void AbbyyReader::MakeTextLanguages()
{
	m_EnglishDictionary = m_FREngine->CreateNewDictionary( m_bstrPathName + L"DTVEnglish.amd", FREngine::LI_EnglishUnitedStates );
	m_EnglishDictionary->Name = "DTVEnglishDictionary";

	m_SpanishDictionary = m_FREngine->CreateNewDictionary( m_bstrPathName + L"DTVSpanish.amd", FREngine::LI_SpanishTraditionalSort );
	m_SpanishDictionary->Name = "DTVSpanishDictionary";
	// Add words to dictionary
	//pDictionary->AddWord( "the", 100 );

    // Create new TextLanguage objects
	m_EnglishTextLanguage = m_FREngine->CreateTextLanguage();
	m_SpanishTextLanguage = m_FREngine->CreateTextLanguage();

	// Copy all attributes from predefined English language
	FREngine::ITextLanguagePtr pEnglishLanguage = m_FREngine->PredefinedLanguages->FindLanguage( "English" )->TextLanguage;
	m_EnglishTextLanguage->CopyFrom( pEnglishLanguage );
	m_EnglishTextLanguage->InternalName = "DTVTextEnglishLanguage";

	FREngine::ITextLanguagePtr pSpanishLanguage = m_FREngine->PredefinedLanguages->FindLanguage( "Spanish" )->TextLanguage;
	m_SpanishTextLanguage->CopyFrom( pSpanishLanguage );
	m_SpanishTextLanguage->InternalName = "DTVTextSpanishLanguage";

	// Bind new dictionary to first (and single) BaseLanguage object within TextLanguage
	FREngine::IBaseLanguagePtr pEnglishBaseLanguage = m_EnglishTextLanguage->BaseLanguages->Item(0);
	FREngine::IBaseLanguagePtr pSpanishBaseLanguage = m_SpanishTextLanguage->BaseLanguages->Item(0);

	// Change internal dictionary name to user-defined
	pEnglishBaseLanguage->InternalName = "DTVTextEnglishLanguage";
	pSpanishBaseLanguage->InternalName = "DTVTextSpanishLanguage";

	// Get collection of dictionary descriptions and remove all items
	FREngine::IDictionaryDescriptionsPtr pEnglishDictionaryDescriptions = pEnglishBaseLanguage->DictionaryDescriptions;
	FREngine::IDictionaryDescriptionsPtr pSpanishDictionaryDescriptions = pSpanishBaseLanguage->DictionaryDescriptions;
	pEnglishDictionaryDescriptions->RemoveAll();
	pSpanishDictionaryDescriptions->RemoveAll();
	
	// Create user dictionary description and add it to the collection
	FREngine::IUserDictionaryDescriptionPtr userEnglishDic = m_FREngine->CreateUserDictionaryDesc();
	FREngine::IUserDictionaryDescriptionPtr userSpanishDic = m_FREngine->CreateUserDictionaryDesc();

	userEnglishDic->FileName = m_bstrPathName + L"DTVEnglish.amd";
	userSpanishDic->FileName = m_bstrPathName + L"DTVSpanish.amd";

	pEnglishDictionaryDescriptions->Add( userEnglishDic );
	pSpanishDictionaryDescriptions->Add( userSpanishDic );

enum _BaseLanguageLetterSetEnum
{
    BLLS_Alphabet = 0,
    BLLS_Prefixes = 1,
    BLLS_Suffixes = 2,
    BLLS_IgnorableLetters = 3,
    BLLS_SubscriptAlphabet = 4,
    BLLS_SuperscriptAlphabet = 5
};
enum _TextLanguageLetterSetEnum
{
    TLLS_InterwordPunctuators = 0,
    TLLS_ProhibitedLetters = 1,
    TLLS_Prefixes = 2,
    TLLS_Suffixes = 3
};
	CComBSTR letters;
	m_EnglishTextLanguage->get_LetterSet( FREngine::TLLS_InterwordPunctuators, &letters.m_str );
	m_EnglishTextLanguage->get_LetterSet( FREngine::TLLS_ProhibitedLetters, &letters.m_str );
	m_SpanishTextLanguage->get_LetterSet( FREngine::TLLS_InterwordPunctuators, &letters.m_str );
	m_SpanishTextLanguage->get_LetterSet( FREngine::TLLS_ProhibitedLetters, &letters.m_str );

	pEnglishBaseLanguage->get_LetterSet( FREngine::BLLS_Alphabet, &letters.m_str );
	pEnglishBaseLanguage->get_LetterSet( FREngine::BLLS_Prefixes, &letters.m_str );
	pEnglishBaseLanguage->get_LetterSet( FREngine::BLLS_Suffixes, &letters.m_str );
	pEnglishBaseLanguage->get_LetterSet( FREngine::BLLS_IgnorableLetters, &letters.m_str );
	pEnglishBaseLanguage->get_LetterSet( FREngine::BLLS_SubscriptAlphabet, &letters.m_str );
	pEnglishBaseLanguage->get_LetterSet( FREngine::BLLS_SuperscriptAlphabet, &letters.m_str );

	pSpanishBaseLanguage->get_LetterSet( FREngine::BLLS_Alphabet, &letters.m_str );
	pSpanishBaseLanguage->get_LetterSet( FREngine::BLLS_Prefixes, &letters.m_str );
	pSpanishBaseLanguage->get_LetterSet( FREngine::BLLS_Suffixes, &letters.m_str );
	pSpanishBaseLanguage->get_LetterSet( FREngine::BLLS_IgnorableLetters, &letters.m_str );
	pSpanishBaseLanguage->get_LetterSet( FREngine::BLLS_SubscriptAlphabet, &letters.m_str );
	pSpanishBaseLanguage->get_LetterSet( FREngine::BLLS_SuperscriptAlphabet, &letters.m_str );
}
*/

/*
//m_RecognizerParams->CellsCount Long 
//m_RecognizerParams->UserPatternsFile String 

//m_RecognizerParams->PossibleTextTypes LongsCollection 
//m_RecognizerParams->TextLanguage TextLanguage 

//m_RecognizerParams->ErrorHiliteLevel ErrorHiliteLevelEnum 
	EHL_None,
	EHL_Scanty,
	EHL_Standard,
	EHL_Thorough,
...
//m_RecognizerParams->FieldMarkingType FieldMarkingTypeEnum 
    FMT_SimpleText,
	FMT_UnderlinedText,
	FMT_TextInFrame,
	FMT_GreyBoxes,
...
//m_RecognizerParams->TextType TextTypeEnum 
	TT_Normal,
	TT_Typewriter,
	TT_Matrix,
	TT_Index,
	...
	TT_Gothic,
	TT_ToBeDetected
//m_RecognizerParams->CaseRecognitionMode
	CRM_AutoCase,
	CRM_SmallCase,
	CRM_CapitalCase

//m_PrepareImageMode->PreviewHeight Long 
//m_PrepareImageMode->PreviewWidth Long 
//m_PrepareImageMode->Rotation RotationTypeEnum 
//m_PrepareImageMode->ImageCompression ImageCompressionEnum 
//m_PrepareImageMode->XResolutionToOverwrite Long 
//m_PrepareImageMode->YResolutionToOverwrite Long 
//m_PrepareImageMode->ColorJpegQuality Long 
//m_PrepareImageMode->GrayJpegQuality Long 
//m_ImageProcessingParams->RotationType RotationTypeEnum 
//m_ImageProcessingParams->BlackGarbageSize Long 
//m_ImageProcessingParams->WhiteGarbageSize Long 
//m_PageAnalysisParams->ColumnAnalysisMode PageColumnAnalysisModeEnum NOT SUPPORTED

//HyperlinksColor Long 

m_PageAnalysisParams->PageSynthesisParams->RecognizedTextFontName String 
	FT_Serif,
	FT_SansSerif,
	FT_MonoSpaceMonospaceDetectionMode MonospaceDetectionModeEnum 
	MDM_Auto,
	MDM_NotMonospace,
	MDM_Monospace
m_PageAnalysisParams->PageSynthesisParams->TextExtractionMode 
	TEM_AutoDetect,
	TEM_Both,
	TEM_RecognitionOnly,
	TEM_PdfInfoOnly
...
/*
        //this does not work as reliably... :( it fails with the very small bitmaps with few chars 
        long byteWidth = (pInfo->bmiHeader.biWidth % 16) ? ((pInfo->bmiHeader.biWidth/16)+1)*16 : pInfo->bmiHeader.biWidth;
        byteWidth /= 8;
        #pragma warning( disable: 4311)
        //pImageDoc = m_FREngine->OpenMemoryImage( FREngine::MIF_BlackAndWhite, pInfo->bmiHeader.biWidth, pInfo->bmiHeader.biHeight, byteWidth, (LONG)m_ImageResolution, (long)(void*)pBitmapBuffer );
        pImageDoc = m_FREngine->PrepareAndOpenMemoryImage( FREngine::MIF_BlackAndWhite, pInfo->bmiHeader.biWidth, pInfo->bmiHeader.biHeight, byteWidth, 
            (LONG)m_ImageResolution, (LONG)m_ImageResolution, (long)(void*)pBitmapBuffer, m_PrepareImageMode );
*/
        // Perform page layout analysis
	    //m_FREngine->AnalyzePage( pImageDoc, 0, pLayout, 0 );
	    ////m_FREngine->AnalyzePage( pImageDoc, 0, pLayout, 0 );
        //pLayout->Blocks->Item( 0 )->Region->SaveToFile("C:\\ReadBlock1.txt");
/*
        // Create new text block
        FREngine::IBlockPtr pBlock = m_FREngine->CreateBlock( FREngine::BT_Text );
	    long cx = pImageDoc->BlackWhiteImage->Width, cy = pImageDoc->BlackWhiteImage->Height;
        pBlock->Region->AddRect( 1, 1, cx-2, cy-2 );

        //pLayout->Blocks->RemoveAll();
        //pLayout->Blocks->Add(pBlock);
        //pLayout->Blocks->Item( 0 )->Region->SaveToFile("C:\\ReadBlock2.txt");

        //FREngine::IBlocksCollectionPtr pBlocksCollection = m_FREngine->CreateBlocksCollection();
	    //pBlocksCollection->Add( pBlock );

	    //m_FREngine->RecognizeBlocks( pImageDoc, 0, pLayout, pBlocksCollection, 0 );
	    m_FREngine->AnalyzeAndRecognizeBlocks( pImageDoc, 0, pLayout, pLayout->Blocks, 0 );
	    //m_FREngine->RecognizePage( pImageDoc, 0, pLayout, 0 );
*/        
/*
        //_bstr_t ocrtext = GetResultsAsString( pLayout );
        long t,l,b,r;
        pLayout->Blocks->Item( 0 )->Region->get_Top(0,&t);
        pLayout->Blocks->Item( 0 )->Region->get_Left(0,&l);
        pLayout->Blocks->Item( 0 )->Region->get_Bottom(0,&b);
        pLayout->Blocks->Item( 0 )->Region->get_Right(0,&r);
        HRESULT res;
*/
...
/*
    ...
    // Turn on mirroring
    //m_PrepareImageMode->MirrorImage = VARIANT_TRUE;


// Get recognition results as string
// Use SysFreeString to free memory used by the string
//    SysFreeString(ocrtext);
_bstr_t GetResultsAsString( FREngine::ILayoutPtr pLayout )
{
	FREngine::IBlocksCollectionPtr pBlocks = pLayout->Blocks;
	_bstr_t resultString;

	for( int i = 0; i < pBlocks->Count; i++ ) {
		FREngine::IBlockPtr pBlock = pBlocks->Item( i );
		resultString += pBlock->GetName() + L" : ";
		switch( pBlock->GetType() ) {
			case FREngine::BT_Text : {
				FREngine::IParagraphsPtr pParagraphs = pBlock->TextBlockProperties->Text->Paragraphs;
				for( int j = 0; j < pParagraphs->Count; j++ ) {
                    resultString += pParagraphs->Item( j )->Text;
				}
				break;
			}
			case FREngine::BT_CheckmarkGroup : {
				for( int j = 0; j < pBlock->CheckmarkGroupProperties->Count; j++ ) {
					FREngine::IBlockPtr pCheckmark = pBlock->CheckmarkGroupProperties->Item( j );
					resultString +=  pCheckmark->Name;
					FREngine::CheckmarkCheckStateEnum state = pCheckmark->CheckmarkBlockProperties->CheckmarkState;
					switch( state ) {
						case FREngine::CMCS_Checked:
								resultString += L"(Checked)";
								break;
						case FREngine::CMCS_NotChecked:
								resultString += L"(NotChecked)";
								break;
						case FREngine::CMCS_Corrected:
								resultString += "(Corrected)";
								break;
					}
					resultString += j < pBlock->CheckmarkGroupProperties->Count - 1 ? L", " : L"";
				}
				break;
			}
			case FREngine::BT_Checkmark : {
				resultString +=  pBlock->Name;
				FREngine::CheckmarkCheckStateEnum state = pBlock->CheckmarkBlockProperties->CheckmarkState;
				switch( state ) {
					case FREngine::CMCS_Checked:
							resultString += L"(Checked)";
							break;

...
STDMETHODIMP CDTVOCRClientCtl::Test(void)
{
	try{
        //TakeScreenShot();
        //Crop(260,170,478,198);
        //SaveImageFileCrop(L"c:\\test.bmp");
        //RecognizeCrop(0,0,0,0,0);


        //Disconnect();
        //Connect();
        //LoadImageFile(L"c:\\crop.bmp");
        //Crop(0,0,250,28);
        //RecognizeCrop(0,4,0,0,0);
        ...
        //LoadImageFile(L"c:\\test.bmp");
        //LoadImageFile(L"c:\\bmp.bmp");
        LoadImageFile(L"C:\\DTVTest\\AutomationClient\\crops\\IconsInfoCrop.bmp");
        //Crop(558,85,700,110);
        Crop(0,0,159,24); //the info area...
        //LoadImageFile(L"C:\\DTVTest\\AutomationClient\\crops\\ToDoListCrop.bmp");
        //Crop(0,0,196,144);
       
        //LoadImageFile(L"C:\\pig.bmp");
        //Crop(532,122,655,146); //the info area...

        //Crop(630,85,700,110); //only the unlock icon
        //Crop(550,85,700,110); //the speaker and unlock icon
        //Crop(458,387,490,411); //the RECORD
        //Crop(44,189,61,384); //menu icons 
        //Crop(45,230,675,448); //the playlist area...
        //Crop(632,389,677,438); //the trickplay area...
        //Crop(550,85,700,110); //the info area...
        //Crop(180,285,400,430); //todo list area...
        //Crop(304,285,400,430); //todo list area...
        
        //SaveImageFileCrop(L"c:\\DTVTest\\crop.bmp");
        //SaveImageFileCrop(L"c:\\pig_crop.bmp");
        //FindBoundingSquaresInCrop();
        //FindMatchingSinglePartIcons();
        //FindMatchingMultiPartIcons();
        //HasIcon(43); //pause icon
        //HasIcon(7); //info unlock icon
        ////WhichIcons(0);
        WhichIcons(10);
...
/*
            ...
            else if (x1 > nx2 || nx1 > x2 || y1 > ny2 || ny1 > y2) 
            { //no collision
...
/*
   if( !pCoord ) {
    ...
    int lowerAvg = 0;
    int upperAvg = 0;
    //calculate average heights, we'll need these values later
    if( ini_options->OCR_REMOVE_ALPHANUM_INVALID_HEIGHT_BY_AVERAGE() ) {
        //map<int,int> upperCase;
        //map<int,int> lowerCase;
                    //lowerCase[charHeight]++;
                    //upperCase[charHeight]++;
        int lowerCount = 0;
        int upperCount = 0;
        long lowerVal = 0;
        long upperVal = 0;
        for ( UINT16 i=0; i<len; i++ )
        {
		    WCHAR c = static_cast<WCHAR> (*pText++);
            int top     = *pCoord++;
            //int left    = 
            *pCoord++;
            int bottom  = *pCoord++;
        ...
        if( lowerCount )
            lowerAvg = lowerVal / lowerCount;
        if( upperCount )
            upperAvg = upperVal / upperCount;
    }

                    pText       = m_OCRServer.GetText();
                    pCoord      = m_OCRServer.GetCharacterCoordinatesArray();
	                charHeight  = 0;
	unsigned int    charWidth   = 0;
	unsigned int    prevSpc     = 0;
	LONG            prevRight   = 0;

    unsigned int minSpaceWidth = ini_options->OCR_INSERT_SPACE_MIN_PIXEL_WIDTH();

#define MAX_SPACE 100 //takes care of some weird error, because chars can be together, or per ocr server, overlapping...
    m_ProcessedTextBuffer[0] = 0;
    WCHAR * pPText = m_ProcessedTextBuffer;
    for ( UINT16 i=0; i<len; i++ )
    {
		WCHAR c = static_cast<WCHAR> (*pText++);
        int top     = *pCoord++;
		...
		prevSpc = left - prevRight;
		if ( prevSpc < 0 || prevSpc > MAX_SPACE || !prevRight )
			prevSpc = 0;

		//sometimes the engine can't see a space, this will add it...
		if ( minSpaceWidth && prevSpc && prevSpc >= minSpaceWidth && !isspace(c) ) {
            *pPText++ = L' ';
		}
    ...
    *pPText++ = 0;
    ...
    //m_ProcessedTextBuffer[len] = 0;
    //m_bstrProcessedText = m_ProcessedTextBuffer;
    WCHAR wc2 = pText[len];
    pText[len] = 0;
    m_bstrProcessedText = pText;
    pText[len] = wc2;
    ...
    if( ini_options->Debug() && ini_options->Verbose() )
    {
        ACE_DEBUG(( LM_DEBUG, ACE_TEXT( "Processed Text '%W' \n" ), m_bstrProcessedText.m_str ));
    }*/
}

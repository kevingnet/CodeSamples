...
MonoBitmapGrid::MonoBitmapGrid(BYTE * pData, USHORT w, USHORT h, std::vector<USHORT> & coordinates, bool recursing, bool increase ): m_Width(w), m_Height(h), m_pData(pData), m_Coordinates(coordinates), m_IncreaseCoordinates(increase) 
{
    ...
    //if ( sm_RecursionLevel > MAX_RECURSION_LEVEL ) {
    //    sm_RecursionLevel--;
    //    return;
    //}
    g_StackPointer = 0;
    ...
    m_x = 0;
    m_y = 0;
    //m_MAX_ARTIFACT_SIZE = ini_options->MAX_ARTIFACT_WIDTH() * ini_options->MAX_ARTIFACT_HEIGHT();
...
/*
    //erase rectangle
    pBoundingRectangle = (m_pData + (m_Width * m_y + m_x ));
    skip = m_Width - w;
    for (int y=0; y < h; y++)
    {
        for (int x=0; x < w; x++)
            *pBoundingRectangle++ = m_BackgroundColor;
        pBoundingRectangle += skip;
    }
*/
    sm_xFactor += m_x;
...
/*    void CalculateBoundingRectangle2()
{
    m_cur_x = (USHORT)((m_pCurrentBusyGridLocation - m_pData) % m_Width);
    m_cur_y = (USHORT)((m_pCurrentBusyGridLocation - m_pData) / m_Width);
    ...
    m_CoordinatesCohesiveGroup.clear();
    flood_fill(m_x, m_y);
    AddCoordinatesToGrid2();
}

void AddCoordinatesToGrid2()
{
    ULONG x1 = m_Width, y1 = m_Height, x2 = 0, y2 = 0;
    std::vector<ULONG>::iterator iter = m_CoordinatesCohesiveGroup.begin();
    while( iter != m_CoordinatesCohesiveGroup.end() )
    {
        ULONG x = *iter >> 16;
    ...
    m_Coordinates.push_back((USHORT)x1);
    m_Coordinates.push_back((USHORT)y1);
    m_Coordinates.push_back((USHORT)x2);
    m_Coordinates.push_back((USHORT)y2);
}
*/

...
/*
int next_start_code(uint32_t * pBuf, int numbytes)
{
  int wordoff = 0;
  int numwords = numbytes / 4;
  while (wordoff < numwords) {
    if (pBuf[wordoff] == USER_DATA_START_CODE) {
      return (4);
    }
    else if (((pBuf[wordoff] & COUNTRY_MASK) == COUNTRY_CODE)
        && (((pBuf[wordoff] & PROVIDER_MASK) >> 8) == PROVIDER_CODE)) {
      return (3);
    }
    wordoff++;
  }
...
#endif
#if 0
    //KG: HACK! here we were only missing a byte, so let's use it anyway
    uint8_t missing = (i + 2) - size;
    if (missing < 2) {
      if (missing == 0) {
        #ifdef DEBUG_VERBOSE
        LDEBUG("Parse053: no more data");
        #endif
        break;
      }
      else {
        #ifdef DEBUG_VERBOSE
        LDEBUG("Parse053: missing %d bytes ", missing);
        #endif
      }
    }
    if (missing != 1) {
...
//        bufDTV.data[(bufDTV.bytes) + 0] = data[i - 1];
        bufDTV.data[(bufDTV.bytes) + 1] = data[i];
        //if (missing == 1) {
        //  bufDTV.data[(bufDTV.bytes) + 2] = 0;
        //}
        //else {
...
//    LINFO("Classify %x -%x +%x", *(p), *(p-1), *(p+1));
//    LINFO("Classify bytes: %d data: %x %x %x %x", bytes,
...
#endif
//    LINFO("Classify size: %d buffer: %x %x %x %x\n\n", size,

...
bool Parse::ParseData(uint8_t *data, uint32_t size, uint32_t pic)
{
  ...
  Buf60853.bytes = 0;
  Buf70853.bytes = 0;
//  type = Classify(data, size, pic, Buf608157, Buf60853, Buf70853,
//      (uint8_t**)&userData, (uint32_t*)&userSize);
...
//  if (!(count % 40)) {
//  if (Buf70853.bytes)
//    LWARNING("ParseData: size: %d --- ## 708 %d ## - 608 53 %d 157 %d",
//        size, Buf70853.bytes, Buf60853.bytes, Buf608157.bytes);
//  }
  //GetDecoderParameters(&DecoderParams);
...
//  if (HasFocus() && (mpPds->HideInternalCC() == false)) {
//    //mpVBImanager->VbiReinsert(pDVS157_608BufInfo, pDVS53_608BufInfo);
//    inserts++;
//  }
//  static uint32_t counter = 0;
//  counter++;
//  if (!(counter % MESSAGELIMIT)) {
//    LTRACE("%d> inserts: %d, processing %d\n",
//    counter, inserts, cc_format_state);
//  }

//  if ((Buf70853.bytes > 0) &&
//      ((type & DigitalMask_053_708) == DigitalMask_053_708)) {
  if (Buf70853.bytes) {
...
//    LWARNING("ParseData: Buf70853.bytes %d", Buf70853.bytes);
  ...
  //if (!(count % 20)) {
//  if (Buf70853.bytes)
//    LCRITICAL("ParseData: 708 %d - state:%d",
//  Buf70853.bytes, cc_format_state);
  //}
//  count++;
#ifdef DEBUG_VERBOSE

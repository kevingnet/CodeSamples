...
#endif
  //another stupid hack!!!, usually (*not!* always!)
  //define window is 1 and the subsequend commands are for 0
  //which is..., so we are going to substract now and see what happens...
  //well, that went great, only, sometimes...
//  def.ID--;
//  if (def.ID > MAX_WINDOWS) {
//#ifdef DEBUG_VERBOSE
//  LWARNING("CommandDefineWindow: Bad ID: ^%d^, setting to zero", def.ID);
//#endif
//    def.ID = 0;
//  }
//#ifdef DEBUG_VERBOSE
//  LWARNING("CommandDefineWindow: NEW ID: ^%d^", def.ID);
//#endif
...
void TDisplay::CommandDelay(uint32_t lapse)
{
  ...
  // LTRACE("CommandDelay: %ld", CommandDelay);
  // Command delay needs to be in ms.
  // uint32_t DelayMS = DelayInTenths * 100;
  //DelayInMS(DelayInTenths * 100);
  //pDisplayQueue->Pause(ms);
  // Clear delay.
  // DelayMS = 0;
}

...
/*
  ...
  _result = _data.GetXml();
log.Info("JakeKnowsEngine WorkerFunction _result" + _result);
DataTableCollection collection = _data.Tables;
log.Info("WorkerFunction: collection.Count = " + collection.Count);
for (int i = 0; i < collection.Count; i++)
{
    DataTable table = collection[i];
    log.Info("WorkerFunction: table: " + table.TableName);
    foreach (DataRow row in table.Rows)
    {
        foreach (var item in row.ItemArray)
        {
            log.Info("WorkerFunction: row: " + item);
        }
    }
}*/
//m_Counter++;
//_data.Tables.Add("this " + this.GetHashCode().ToString() + " task " + m_TaskID + " thread " + Thread.CurrentThread.GetHashCode().ToString() + " count " + m_Counter.ToString());
//return _data;
